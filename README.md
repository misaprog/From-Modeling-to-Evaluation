...
先にテスト用のサンプル（bamboo_test）を作成したので、
元データから「テストに使う分以外のデータ」を 訓練用データ（bamboo_train） として分けている
さらに、訓練データを材料（特徴量）とジャンル（ラベル）に分割して機械学習などに使いやすくしている

---
### 各料理のレシピが30個減っていることを確認します
```
bamboo_train["cuisine"].value_counts()
```
解説

bamboo_train は、元のデータ bamboo からテスト用のサンプル（bamboo_test）を除外した訓練用データのデータフレームです。
.value_counts() は指定した列内の各値の出現回数をカウントするメソッドです。
したがって、このコードは 訓練データ bamboo_train の中で、各料理ジャンル（cuisine）が何件ずつ存在するかをカウントして表示します。

目的

「訓練データに含まれるジャンルごとの件数分布を確認」し、テストデータと合わせて、データのバランスがどうなっているかを把握するために使います。
